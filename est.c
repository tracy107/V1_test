...
int main(void)
{
	...
	char param_name[5 + 1][30];

	FILE *inp, *para_inp; //, *outp;
	double *param, *jump, *ap, ln_prob_a, eps, acc, th_prob_a = 1.0 / 3;//maxs, mins, 
	...
	jump = dvector(1, n_param);
	ap = dvector(1, n_param);
	...
	llh.mi = dmatrix(1, n_proj, 1, n_person);
	pllh.mi = dmatrix(1, n_proj, 1, n_person);
	llh.i = dvector(1, n_person);
	pllh.i = dvector(1, n_person);

	psd.rs = darray3d(1, n_t, 3, n_pvmh);
	psd.pvf = darray4d(1, n_person, 1, n_proj,  1, n_t, 1, n_pvmh);

	var.vf = darray4d(1, n_person, 1, n_proj,  2, n_t, 1, n_rnd);
	...
	para_inp = fopen("param_input.txt", "r");
	i = 1;
	...
	printf("read rows number =%d  \n",count);
	/*  
	//print out to check whether reading is correct;
	for (i = 1; i <= n_person; i++) {
		for (m = 1; m <= n_proj; m++) {
			for (t = 1; t <= n_t; t++) {			
